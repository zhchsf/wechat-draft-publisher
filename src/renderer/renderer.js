(function () {
  ...
  function issueCount(article) {
    ...
    return (article.issues || []).filter((item) => !ignored.has(item.id)).length;
  }
  ...
  async function inspectSelectedSources(paths) {
    ...
    try {
      ...
      const existing = new Set(state.queue.map((article) => article.sourcePath));
      const additions = articles.filter((article) => !existing.has(article.sourcePath));
  ...
  function selectedArticleIds() {
    return state.queue.filter((article) => ui.selectedIds.has(article.id)).map((article) => article.id);
  }
  ...
  function renderQueueSidebar() {
    ...
    return `<aside class="queue-sidebar"><div class="sidebar-heading"><div><span class="eyebrow">WORKSPACE</span><h1>文章队列</h1></div><button class="icon-button" data-import title="选择 HTML 文件或目录" aria-label="选择 HTML 文件或目录">＋</button></div><div class="sidebar-actions"><button class="primary-button" data-import>导入文章</button><button class="quiet-button" data-select-all>${selectedCount === state.queue.length && state.queue.length ? "取消全选" : "全选"}</button></div><div class="queue-summary"><span>${selectedCount} 篇已选</span><span>${state.queue.filter((article) => issueCount(article)).length} 篇待处理</span></div><div class="queue-list">${state.queue.length ? state.queue.map((article) => `<button class="queue-item ${article.id === ui.activeArticleId ? "active" : ""}" data-article-id="${article.id}"><span class="queue-check ${ui.selectedIds.has(article.id) ? "checked" : ""}" data-select-check="${article.id}">${ui.selectedIds.has(article.id) ? "✓" : ""}</span><span class="queue-item-copy"><strong>${escapeHtml(article.title || basename(article.sourcePath))}</strong><small>${escapeHtml(basename(article.sourcePath))}</small></span><span class="queue-item-status status-${article.status}">${statusLabel(article.status)}</span></button>`).join("") : `<div class="empty-sidebar"><div class="empty-icon">＋</div><strong>还没有文章</strong><p>从 HTML 文件或文章目录开始</p></div>`}</div><div class="sidebar-footnote"><span class="status-dot"></span>本地处理 · 数据保存在本机</div></aside>`;
  }
  ...
  function renderIssues(article) {
    ...
    const ignored = new Set(article.ignoredIssueIds || []);
    return `<div class="issues-box"><div class="section-label"><span>资源检查</span><em>${issues.length - [...ignored].filter((id) => issues.some((issue) => issue.id === id)).length} 项待处理</em></div>${issues.map((issue) => `<label class="issue-row ${ignored.has(issue.id) ? "ignored" : ""}"><input type="checkbox" data-issue-toggle="${issue.id}" ${ignored.has(issue.id) ? "checked" : ""} ${issue.canIgnore ? "" : "disabled"}/><span class="issue-icon">${issue.canIgnore ? "!" : "×"}</span><span><strong>${escapeHtml(issue.message)}</strong><small>${issue.canIgnore ? "勾选后忽略并移除问题资源" : "必须修复后才能发布"}</small></span></label>`).join("")}</div>`;
  }
  ...
  function renderEditor(article) {
    ...
    const validation = issueCount(article) === 0 && article.coverAssetId && String(article.title || "").trim();
  ...
  function render() {
    ...
    app.innerHTML = `${renderHeader()}${view}${ui.busy ? `<div class="busy-indicator"><span></span>正在处理…</div>` : ""}${ui.toast ? `<div class="toast toast-${ui.toast.tone}">${escapeHtml(ui.toast.message)}</div>` : ""}${renderModal()}`;
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (target.matches("[data-select-all]")) {
      if (selectedArticleIds().length === state.queue.length) state.queue.forEach((article) => ui.selectedIds.delete(article.id));
      else state.queue.forEach((article) => ui.selectedIds.add(article.id));
      render();
      return;
    }
    if (target.matches("[data-open-source]")) {
      const article = currentArticle();
      if (article) await api.openSourceFolder(article.sourcePath);
      return;
    }
    if (target.matches("[data-remove-article]")) {
      ...
      ui.activeArticleId = state.queue[0]?.id || null;
      await persistQueue();
      render();
      return;
    }
  ...
  api.onPublishProgress((progress) => {
    ...
    if (article) {
      ...
      if (progress.status === "success") article.draftMediaId = progress.mediaId;
